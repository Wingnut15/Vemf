...
class CfgVemfReloaded
	{
		...
		maxGlobalMissions = 3; // enable/disable global mission amount limit
		...
		class Exile // Exile specific settings
			{
				...
				crateMoney = 20000; // (max) amount of money in the loot crate
				...
				class DynamicLocationInvasion // DynamicLocationInvasion (mission) settings
					{
						...
						groupCount[] = { 4, 6 }; // In format: {minimum, maximum}; VEMF will pick a random number between min and max. If you want the same amount always, use same numbers for minimum and maximum.
						groupUnits[] = { 6, 8 }; // How much units in each group. Works the same like groupCount
						hasLauncherChance = 25; // In percentage. How big the chance that each AI gets a launcher

						class heliPatrol
							{
								enabled = no;
								classesVanilla[] = { "B_Heli_Light_01_armed_F", "B_Heli_Attack_01_F", "B_Heli_Transport_01_F", "B_Heli_Transport_01_camo_F"}; // the types of heli(s)
								classesHeliDLC[] = { };
